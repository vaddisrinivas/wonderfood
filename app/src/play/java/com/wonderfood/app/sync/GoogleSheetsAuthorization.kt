...
object GoogleSheetsAuthorization {
    ...
    fun accessToken(result: AuthorizationResult): String =
        result.accessToken?.takeIf { it.isNotBlank() }
            ?: error("Google Sheets did not return an access token. Try connecting again.")
            ...
            .addOnSuccessListener { authResult ->
                if (authResult.hasResolution()) {
                    val pendingIntent = authResult.pendingIntent ?: run {
                        onFailure(IllegalStateException("Google Sheets permission prompt was unavailable."))
