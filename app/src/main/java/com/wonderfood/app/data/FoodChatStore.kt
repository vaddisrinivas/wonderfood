    ...
    private fun readInventory(): List<InventoryItem> =
        readableDatabase.rawQuery(
            ...
            SELECT id, name, quantity, zone, category, serving_text, calories, protein_g, carbs_g, fat_g,
                   nutrition_source, notes, image_uri, image_url, expires_at, source, created_at, updated_at
            FROM inventory_items WHERE archived = 0 ORDER BY id DESC
            """.trimIndent(),
            emptyArray(),
        ).useRows { cursor ->
            InventoryItem(
    ...
    private fun ensureInventoryBatch(
        ...
        return db.insert(
            ...
            ContentValues().apply {
                ...
                put("original_quantity_value", parsed.value)
                put("storage_zone_id", storageZoneId)
                putNull("purchase_date_epoch_day")
                putNull("opened_date_epoch_day")
                if (expiresAtMillis == null) putNull("expires_date_epoch_day") else put("expires_date_epoch_day", expiresAtMillis.toEpochDay())
                putNull("best_before_date_epoch_day")
                putNull("purchase_price_cents")
...
fun categorizeFood(value: String): String {
    ...
    return when {
        ...
        listOf("spinach", "lettuce", "greens", "broccoli", "carrot", "pepper", "tomato", "bhindi", "okra", "lauki", "gourd", "brinjal", "eggplant").any { it in text } -> "produce"
