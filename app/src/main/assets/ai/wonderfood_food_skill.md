...
A receipt is evidence of a completed purchase. Never default receipt items to the
grocery/to-buy list.

Return a `receipt` review draft that accounts for every visible line:

- Purchased food: `disposition=INVENTORY` with `FRIDGE`, `FREEZER`, or `PANTRY`.
- Household/non-food product: `disposition=HOUSEHOLD`; keep it out of food inventory.
- Tax, payment, subtotal, discount, coupon, and unreadable control lines:
  `disposition=IGNORE`.

Also capture merchant, store location, purchase date, ISO currency code, line prices,
subtotal, tax, and total when visible. Emit monetary values as integer minor units
(`line_price_cents`, `subtotal_cents`, `tax_cents`, `total_cents`). Never infer an
unreadable price or store location.

For each purchased food, return the visible receipt line as evidence, quantity when
visible, canonical category, emoji, storage zone, confidence, and warnings. Nutrition
may be estimated only when the product and serving are sufficiently recognizable;
use `nutrition_source=ai_estimate`. Otherwise keep nutrition null. A best-before may
be conservatively estimated, but it must use `expiry_source=ai_shelf_life_estimate`
and warn the user to check the package. Never represent an estimate as a printed date.

All receipt items remain editable before acceptance. Acceptance puts away only
`INVENTORY` lines. It never silently adds `HOUSEHOLD` or `IGNORE` lines to Kitchen.
