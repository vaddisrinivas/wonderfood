...
export default function SearchScreen() {
  ...
  const results = useMemo(() => foodRecords.filter((item) => `${item.title} ${item.meta} ${item.body}`.toLowerCase().includes(query.toLowerCase())), [query]);
  const items: any[] = query ? results : commands;
  return <Page><View style={sharedStyles.content}>
    ...
    <ScrollView keyboardShouldPersistTaps="handled"><Text style={styles.label}>{query ? `${results.length} MATCHES` : 'QUICK ACTIONS'}</Text><Card style={styles.list}>
      {items.map((item) => {
        const href: any = query ? { pathname: '/record/[id]', params: { id: item.id } } : item.href;
        return <Link href={href} asChild key={item.id}><Pressable style={styles.result}><View style={styles.resultIcon}><Text>{query ? '◉' : item.icon}</Text></View><View style={{ flex: 1 }}><Text style={styles.resultTitle}>{item.title}</Text><Text style={styles.resultDetail}>{query ? item.meta : item.detail}</Text></View>{query ? <Pill tone={item.tone}>{item.status}</Pill> : <Text style={styles.chevron}>›</Text>}</Pressable></Link>;
      })}
      {query && results.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>Nothing exact yet</Text><Text style={styles.resultDetail}>Ask LifeOS to search connected sources or the web.</Text><Link href="/(tabs)/chat" style={styles.ask}>Ask with AI →</Link></View> : null}
    </Card></ScrollView>
  </View></Page>;
}
