...
| 8 — Android completion | Agent G | IN PROGRESS | Expo native project generated under `android/`; release APK `android/app/build/outputs/apk/release/app-release.apk` launches on emulatorx and renders the Today dashboard (`app/build/evidence/emulatorx-release-durable.png`); API 34 emulatorx includes the Health Connect controller/onboarding; `src/health/connect.ts`; Health Connect read-only manifest permissions and permission delegate; authenticated snapshot sync routes in `server/src/health/snapshots.ts`; `scripts/quality/check-health-connect-android.sh` PASS | Emulatorx permission-denied state is verified and controller onboarding is reachable; app-specific permission grant/read/export/delete, background scheduling, and visible settings trigger remain open | Exercise granted flow on emulatorx and wire a visible settings trigger |
