...
| 3 — Direct AI chat | S2 | PARTIAL — SOURCE-CONTEXT/RICH-RENDER SLICE | `src/settings/lifeos-settings.ts`, `src/chat/{client,direct-provider,types,citations}.ts`, `app/(tabs)/chat.tsx`, `app/settings.tsx`, `app/config.tsx`; historical server-side chat proofs remain scoped evidence | Direct provider receives local source excerpts and renders generic tables/source cards; real primary/fallback key proof, ten-turn correction, web-search citations in direct-provider mode, and second-device resume remain open | Prove primary/fallback on emulator with user key, then add direct-provider web-search/source retrieval controls |
...
| 5 — Optional Notion adapter | PARTIAL — DIRECT APP PULL UI WIRED | `server/src/providers/notion/*`, `src/providers/direct-source-sync.ts`, `app/sources.tsx`, sync/webhook files, adapter tests; historical disposable scenario evidence at `app/build/evidence/live-workspace/notion_scenarios-1784760281.json` | Full template counts/relations/rollups, real user-token native pull proof, provider Undo, permission-loss recovery, standalone Notion loop, disconnect, and durable visual proof remain open | Prove Notion-only authority and Notion-disabled modes with user-owned credentials; keep webhooks optional |
| 6 — Optional Google Sheets adapter | PARTIAL — DIRECT APP PULL UI WIRED | `server/src/providers/sheets/*`, `src/providers/direct-source-sync.ts`, `app/sources.tsx`, sync/webhook files, adapter tests; historical live workbook scenario evidence | Real user-token workbook pull proof, formula/user-column preservation beyond raw property payload, provider Undo, round trip, disconnect, and Sheets-only visual UX remain open | Prove Sheets-only authority and Sheets-disabled modes with user-owned OAuth/workbook |
