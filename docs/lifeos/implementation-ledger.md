...
| 6 — Google Sheets adapter | PASS (ADAPTER + LIVE DISPOSABLE) | `server/src/providers/sheets/*`, `server/src/providers/sync/sheets.ts`, `server/src/providers/webhooks/sheets.ts`, `server/src/index.ts`, `server/test/provider-sync-sheets.ts`, `server/test/provider-webhook-ingress.ts`, `scripts/quality/check-phase6-sheets-adapter.ts`; local evidence at `app/build/evidence/phase6-sheets-adapter/phase6-sheets-adapter-proof.json`; disposable live evidence at `app/build/evidence/live-workspace/google_sheets_scenarios-1784753699.json` | Server production ingress still needs configured canonical workbook/data-source IDs; authority gate prevents Sheets overwriting Notion by default | Configure approved canonical workbook and run server webhook → pull → authority-gated canonical-state live proof |
