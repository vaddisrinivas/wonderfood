...
| 3 — Server and real chat | Agent B (Chat/Responses/Conversations) | IN PROGRESS | `server/src/{chat.ts,conversations.ts,conversations.ts,index.ts,actions.ts,providers/openai.ts,provenance.ts,agents/*}.ts`, `src/chat/*.ts`, `97e9dc1`, `7bbe263` | Multi-agent policy path, streaming cancellation tokens, and durable conversation history beyond in-memory still incomplete; file-backed storage recovery works but not yet tied to canonical persistence contract or action-level idempotency across server restarts | Finish run-chat provenance + action/receipt contract, then add full response streaming + model retry/compensation tests |
