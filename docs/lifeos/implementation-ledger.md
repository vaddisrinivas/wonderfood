...
- `2026-07-22`: fixed hosted MCP static-resource resolution when launched with `npm --prefix server`; live resource reads previously returned `Missing resource file`, and root discovery now serves the catalog, Food skill, and schemas from the same paths as local MCP.
